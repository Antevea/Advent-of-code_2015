...
fn count_lit_lights(light_grid: [[u16; 1000]; 1000]) -> u64 {
    let mut lit_lights: u64 = 0;

    for row in &light_grid[..] {
        for light in &row[..] {
            lit_lights += *light as u64;
        }
    }
...
fn part1() {
        ...
        for row in &mut light_grid[start_x..=end_x] {
            for light in &mut row[start_y..=end_y] {
                *light = match lightflag {
                    LightswithFlags::LightOn => 1,
                    LightswithFlags::LightOff => 0,
                    LightswithFlags::LightToggle => if *light == 1 {0} else {1},
                }
            }
        }
    }
    ...
    let mut light_grid = [[0 as u16; 1000]; 1000];
...
fn part2() {
        ...
        end_coord: Vec<u16>)
    {
        ...
        for row in &mut light_grid[start_x..=end_x] {
            for light in &mut row[start_y..=end_y] {
                *light = match lightflag {
                    LightswithFlags::LightOn => *light + 1,
                    LightswithFlags::LightOff => if *light <= 1 {0} else {*light - 1},
                    LightswithFlags::LightToggle => *light + 2,
                }
            }
        }
    }
    ...
    let instructions_vec: Vec<&str> = input_str.trim_end().split('\n').collect::<Vec<&str>>();
    let mut light_grid = [[0u16; 1000]; 1000];
    ...
    let total_brightness = count_lit_lights(light_grid);
